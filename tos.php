			...
			if(1==1)
			{
					...
					else//เริ่มทำงาน
					{
						...
						if($numrows > 0)
						{
							...
							if($return_cmd == "ADDUSER")
							{
								$messages = [
								'type' => 'text',			
								'text' => 'R3 กรุณาใส่ชื่อของคุณ'//.json_encode($event)
								];
								$messagesX[0] = $messages;
				...
				else
				{
					...
					if($numrows_all > 0 && $numrows <= 0)
					{
						...
						while ($row = pg_fetch_row($result_all)) 
						{					
					...
					{	
						...
						$messages = [
						'type' => 'text',			
						'text' => 'R3 พร้อมเริ่ม Loop ใหม่ '//.$del_loop//.pg_fetch_result($result, 0, 4).' ins_cmd='.$chk_opencmd
						];
						$messagesX[0] = $messages;
					}
					else
