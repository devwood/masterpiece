		...
		if ($event['type'] == 'message' && $event['message']['type'] == 'text') 
		{
			...
			elseif(strtoupper($text) == 'ONLINE POS')
			{
				...
				else
				{
					$messages = [
					'type' => 'text',			
					'text' => 'R5 ไม่มีข้อมูล POS Online ใน 5 นาทีนี้:'.$know
					];
					$messagesX[0] = $messages;
				}
...
function _resultXQUERY($text, $dbconn, $event, $access_token)
{
	...
	//if(1==1)//ใช้ไปก่อน
	{
		...
		{	
			...
			$messages = [
			'type' => 'text',			
			'text' => 'X R5='.$return." ไปยัง ".$cmd_to." ด้วยคำสั่ง ".$ins_cmd
			];
			$messagesX[0] = $messages;
			$numrows = 1;
		}
